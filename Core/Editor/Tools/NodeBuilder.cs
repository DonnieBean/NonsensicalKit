...
";
        ...
        private static string className
        {
            get
            {
                return StringHelper.GetFileNameByPath(scriptPath);
            }
        }
        ...
        private void DrawLeft()
        {
...

            {
                ...
                {
                    string jsonData = JsonHelper.SerializeObject(classBuilder.rootNode);
        ...
        public void MountScript()
        {
            ...
            var scriptType = ReflectionHelper.GetAssembly().GetType(className);
            if (scriptType == null)
            {
            ...
            //此处由于预制体机制大幅修改，需要重新研究

            //var obj = PrefabUtility.GetCorrespondingObjectFromSource(TBTManager.selectTransform.gameObject);
            //if (obj != null)
            //{
            //    PrefabUtility.SaveAsPrefabAsset(TBTManager.selectTransform.gameObject, obj, ReplacePrefabOptions.ConnectToPrefab);
            //}
        ...
        private class ClassBuilder
        {
            ...
            {
                this.raw = raw;
                rootNode = JsonHelper.DeserializeObject<NodeInfo>(jsonText);
            }
            ...
            public class NodeInfo
            {
                ...
                public NodeInfo(Transform t, string path)
                {
                    ...
                    name = t.name;
                    this.path = path;
                    components = GameObjectHelper.GetComponentsName(t);
                    CheckAbbreviation();
                    ChangeType();
                ...
                public string GetAwakeString()
                {
                    if (useFind == true)
                    {
                        ...
                        {
                            sb.Append("transform");
                            sb.Append(GetComponentString(components[choiceComponent]));
                            sb.Append(";\n");
                        }
                        else
                        {
                ...
                private string GetComponentString(string name)
                {
                    StringBuilder sb = new StringBuilder();
                    if (name == "GameObject")
                    {
